...
public final class OffxHomeWidgetOrderStore {
    private static final String PREFS = "offx_home_widget_order";
    ...
    public static void sync(Context context, ArrayList<Integer> ids) {
        Set<String> next = new LinkedHashSet<>();
        ArrayList<Integer> current = load(context);
        for (Integer id : current) if (ids.contains(id)) next.add(String.valueOf(id));
        for (Integer id : ids) if (!next.contains(String.valueOf(id))) next.add(String.valueOf(id));
        save(context, next);
    }

    public static void move(Context context, int id, int direction) {
        ArrayList<Integer> list = load(context);
        int at = list.indexOf(id);
        int to = at + direction;
        if (at < 0 || to < 0 || to >= list.size()) return;
        Integer other = list.get(to);
        list.set(at, other);
        list.set(to, id);
        Set<String> next = new LinkedHashSet<>();
        for (Integer value : list) next.add(String.valueOf(value));
        save(context, next);
    }
