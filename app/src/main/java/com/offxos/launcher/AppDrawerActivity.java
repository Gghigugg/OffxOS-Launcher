...
public class AppDrawerActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().setStatusBarColor(Color.TRANSPARENT);getWindow().setNavigationBarColor(Color.BLACK);recent=getSharedPreferences("offx_recent",MODE_PRIVATE);performanceStore=new PerformanceStore(this);build();}
    @Override public void onResume(){super.onResume();if(grid!=null&&search!=null)render(search.getText().toString());}
    int motion(int normal,int lite){return performanceStore.isLiteMode()?lite:normal;}
    void build(){
        LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(16),dp(30),dp(16),dp(12));root.setBackgroundColor(0xFF08090D);root.setAlpha(0f);root.setTranslationY(dp(18));
        ...
        TextView close=tv("✕",20);close.setBackground(glass());close.setOnClickListener(v->{v.animate().scaleX(.82f).scaleY(.82f).setDuration(motion(70,45)).withEndAction(()->finish()).start();});head.addView(close,new LinearLayout.LayoutParams(dp(48),dp(48)));root.addView(head);
        ...
        ScrollView scroll=new ScrollView(this);grid=new LinearLayout(this);grid.setOrientation(LinearLayout.VERTICAL);grid.setPadding(0,dp(6),0,dp(20));scroll.addView(grid);root.addView(scroll,new LinearLayout.LayoutParams(-1,0,1));setContentView(root);buildChips();loadApps();
        search.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int c,int d){}public void onTextChanged(CharSequence s,int a,int b,int c){render(s.toString());}public void afterTextChanged(Editable e){}});installSwipe(root);
        root.animate().alpha(1f).translationY(0).setDuration(motion(260,120)).setInterpolator(new DecelerateInterpolator()).start();
    }
    void buildChips(){chips.removeAllViews();String[] cs={"All","Recent","Social","Media","Tools","Games","System"};for(String c:cs){TextView b=tv(c,12);b.setPadding(dp(16),0,dp(16),0);b.setBackground(chipBg(c.equals(category)));b.setOnClickListener(v->{category=c;v.animate().scaleX(.94f).scaleY(.94f).setDuration(motion(60,35)).withEndAction(()->{v.animate().scaleX(1f).scaleY(1f).setDuration(motion(150,80)).setInterpolator(new OvershootInterpolator(1.4f)).start();buildChips();render(search.getText().toString());}).start();});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,dp(40));lp.setMargins(0,dp(2),dp(8),dp(2));chips.addView(b,lp);}}
    ...
    void render(String filter){grid.removeAllViews();String f=filter==null?"":filter.toLowerCase(Locale.getDefault());ArrayList<AppInfo> shown=filtered(f);if(shown.isEmpty()){TextView e=tv(category.equals("Recent")?"No recently opened apps":"No apps found",15);e.setTextColor(0xAAFFFFFF);e.setAlpha(0f);grid.addView(e,new LinearLayout.LayoutParams(-1,dp(100)));e.animate().alpha(1f).setDuration(motion(180,90)).start();return;}if(category.equals("Recent")&&f.isEmpty()){TextView t=tv("Recently opened",13);t.setGravity(Gravity.LEFT|Gravity.CENTER_VERTICAL);t.setTextColor(0x88FFFFFF);grid.addView(t,new LinearLayout.LayoutParams(-1,dp(30)));}LinearLayout row=null;for(int i=0;i<shown.size();i++){if(i%4==0){row=new LinearLayout(this);row.setGravity(Gravity.CENTER);grid.addView(row,new LinearLayout.LayoutParams(-1,dp(96)));}addApp(row,shown.get(i),i);}}
    void addApp(LinearLayout parent,AppInfo a,int index){LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setGravity(Gravity.CENTER);box.setAlpha(0f);box.setScaleX(.92f);box.setScaleY(.92f);ImageView iv=new ImageView(this);iv.setImageDrawable(a.icon);iv.setPadding(dp(2),dp(2),dp(2),dp(2));box.addView(iv,new LinearLayout.LayoutParams(dp(54),dp(54)));TextView name=tv(a.label,10);name.setSingleLine();box.addView(name,new LinearLayout.LayoutParams(dp(78),dp(22)));box.setOnClickListener(v->{recent.edit().putLong(a.pkg,System.currentTimeMillis()).apply();v.animate().scaleX(.88f).scaleY(.88f).setDuration(motion(75,45)).setInterpolator(new DecelerateInterpolator()).withEndAction(()->{v.animate().scaleX(1f).scaleY(1f).setDuration(motion(220,100)).setInterpolator(new OvershootInterpolator(1.6f)).withEndAction(()->{try{startActivity(a.intent);}catch(Exception ignored){}}).start();}).start();});parent.addView(box,new LinearLayout.LayoutParams(0,-1,1));box.postDelayed(()->box.animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(motion(220,100)).setStartDelay(performanceStore.isLiteMode()?0:Math.min(index*18,180)).setInterpolator(new DecelerateInterpolator()).start(),20);}
    void installSwipe(View target){final float[] sy={0};target.setOnTouchListener((v,e)->{if(e.getAction()==MotionEvent.ACTION_DOWN){sy[0]=e.getY();return false;}if(e.getAction()==MotionEvent.ACTION_UP&&e.getY()-sy[0]>dp(100)){v.animate().alpha(0f).translationY(dp(24)).setDuration(motion(180,90)).setInterpolator(new DecelerateInterpolator()).withEndAction(this::finish).start();return true;}return false;});}
