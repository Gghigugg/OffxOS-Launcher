...
public class AppDrawerActivity extends Activity {
    ...
    void build(){
        ...
        TextView close=tv("✕",20);close.setBackground(glass());close.setOnClickListener(v->{v.animate().scaleX(.82f).scaleY(.82f).setDuration(70).withEndAction(()->finish()).start();});head.addView(close,new LinearLayout.LayoutParams(dp(48),dp(48)));root.addView(head);
        ...
        root.animate().alpha(1f).translationY(0).setDuration(260).setInterpolator(new DecelerateInterpolator()).start();
    }
    void buildChips(){chips.removeAllViews();String[] cs={"All","Recent","Social","Media","Tools","Games","System"};for(String c:cs){TextView b=tv(c,12);b.setPadding(dp(16),0,dp(16),0);b.setBackground(chipBg(c.equals(category)));b.setOnClickListener(v->{category=c;v.animate().scaleX(.94f).scaleY(.94f).setDuration(60).withEndAction(()->{v.animate().scaleX(1f).scaleY(1f).setDuration(150).setInterpolator(new OvershootInterpolator(1.4f)).start();buildChips();render(search.getText().toString());}).start();});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,dp(40));lp.setMargins(0,dp(2),dp(8),dp(2));chips.addView(b,lp);}}
