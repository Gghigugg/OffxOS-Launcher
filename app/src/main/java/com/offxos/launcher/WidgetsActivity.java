...
public class WidgetsActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);host=new OffxWidgetHost(this);build();}

    @Override protected void onStart(){super.onStart();try{host.startListening();}catch(Exception ignored){}restoreWidgets();}
    @Override protected void onStop(){try{host.stopListening();}catch(Exception ignored){}super.onStop();}
    ...
    void pickWidget(){
        ...
        i.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID,pendingId);
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){super.onActivityResult(requestCode,resultCode,data);
        if(requestCode==REQ_PICK){
            ...
            if(m.bindAppWidgetIdIfAllowed(pendingId,info.provider)) finishBinding(info);
    ...
    void addWidgetCard(int id,AppWidgetProviderInfo info,boolean restored){
        android.appwidget.AppWidgetHostView view=host.createView(this,id,info);
        view.setPadding(dp(8),dp(8),dp(8),dp(8));
        view.setBackground(glass());
        TextView remove=tv("Remove widget",12);remove.setGravity(Gravity.CENTER);remove.setBackground(glass());
